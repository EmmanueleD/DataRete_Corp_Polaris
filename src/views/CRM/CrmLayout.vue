      ...
      <Card class="mb-4">
        <template #content>
          <div class="flex flex-column ml-4">
            <h2 v-if="currentCrm.nome && currentCrm.cognome">
              {{ currentCrm.nome + " " + currentCrm.cognome }}
            </h2>
            <span>{{ currentCrm.email }}</span>
          </div>
        </template>
      </Card>
  ...
  service
    ...
    .then((res) => {
      ...
      if (statusSelected.value.id == 14) {
        ...
        axios
          .post("https://hooks.zapier.com/hooks/catch/12276670/3oehybs/", {
