...
async function getViews() {
  ...
  await service
    .readCustomEndpoint(
      "https://polaris-core.datarete.cloud/api/Tables/GetTableInfo/1"
    )
    .then((res) => {
      if (res) {
...
async function getColumns(idVista) {
  ...
  if (idVista) {
    await service
      .readCustomEndpoint(
        "https://polaris-core.datarete.cloud/api/Tables/GetTableView/1/" +
          idVista
      )
      .then((res) => {
        if (res) {
...
function getTableItems() {
  ...
  // axios.get('https://polaris-core.datarete.cloud/api/Anagrafiche/Retail', {
  //   params: {
